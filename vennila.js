...
function clickHandler() {
    ...
    if (checkBday === true) {
        ...
        var prev = findingPalindromForPreviousDays(date)
        var nxt = findingPalindromForNextDays(date)
        if (nxt[0] < prev[0]) {
            outPutDisplay(nxt[0]+" days left for next palindrom🤩🤩🔥🔥");
        } else {
            outPutDisplay(" You missed "+prev[0]+" days😕😕 from palindrom date.")
